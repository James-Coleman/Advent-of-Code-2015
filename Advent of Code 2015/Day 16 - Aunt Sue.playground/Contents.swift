...
struct Sue {
    ...
    init(number: Int?, children: Int?, cats: Int?, samoyeds: Int?, pomeranians: Int?, akitas: Int?, vizslas: Int?, goldfish: Int?, trees: Int?, cars: Int?, perfumes: Int?) {
        self.number = number
    ...
    init(string: String) {
        let components = string.split { character in
            character == " " || character == ":" || character == ","
        }.map { String($0) }
        
//        print(components)
        
        self.number = Int(components[1])
        
        var localChildren: Int? = nil
        var localCats: Int? = nil
        var localSamoyeds: Int? = nil
        var localPomeranians: Int? = nil
        var localAkitas: Int? = nil
        var localVizslas: Int? = nil
        var localGoldfish: Int? = nil
        var localTrees: Int? = nil
        var localCars: Int? = nil
        var localPerfumes: Int? = nil
        
        for index in 2..<components.count {
            guard index % 2 == 0 else { continue } // Only do the even numbers
            
            switch components[index] {
                case "children":
                    localChildren = Int(components[index + 1])
                case "cats":
                    localCats = Int(components[index + 1])
                case "samoyeds":
                    localSamoyeds = Int(components[index + 1])
                case "pomeranians":
                    localPomeranians = Int(components[index + 1])
                case "akitas":
                    localAkitas = Int(components[index + 1])
                case "vizslas":
                    localVizslas = Int(components[index + 1])
                case "goldfish":
                    localGoldfish = Int(components[index + 1])
                case "trees":
                    localTrees = Int(components[index + 1])
                case "cars":
                    localCars = Int(components[index + 1])
                case "perfumes":
                    localPerfumes = Int(components[index + 1])
                default:
                    continue
            }
        }
        
        self.children = localChildren
        self.cats = localCats
        self.samoyeds = localSamoyeds
        self.pomeranians = localPomeranians
        self.akitas = localAkitas
        self.vizslas = localVizslas
        self.goldfish = localGoldfish
        self.trees = localTrees
        self.cars = localCars
...
let realSue = Sue(number: nil, children: 3, cats: 7, samoyeds: 2, pomeranians: 3, akitas: 0, vizslas: 0, goldfish: 5, trees: 3, cars: 2, perfumes: 1)

let testSue = Sue(string: "Sue 1: children: 1, cars: 8, vizslas: 7")
//print(testSue)
